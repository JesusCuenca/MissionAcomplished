...
public class LobbyController : MonoBehaviour
{
    ...
    private void CreateRoom() {
        NetworkClient.Lobby.CreateRoom("", false, 4, 0, true, GameIndex.MakeRandomIndexData(), (bool success, string roomId, SWLobbyError error) => {
            if (!success)
            {
    ...
    private void FindAndJoinRoom() {
        ...
        NetworkClient.Lobby.FilterRoom(GameIndex.MakeFilterData(this.gameName), 1, (success, reply, error) => {
            if (!success)
            {
